<script lang="ts">
    ...
    onMount(async function currentdata() {
        ...
        if (typeof fetch !== "undefined") {
            // const url01 = `${link}/Get/Dosing/Summary/Detail_filter.php`;
            // fetch(url01) // Send a GET request to the specified URL
            //     .then((response) => response.json())
            //     .then((json) => (items = json.slice(0)));
            const url02 = `${link}/Get/Dosing/Summary/Summary_filter.php`;
    ...
    async function searching() {
        ...
        if (typeof fetch !== "undefined") {
            ...
            const url1 = `${link}/Get/Dosing/Summary/Detail_filter.php?date_get1=${start}&&date_get2=${stop}
    ...
    async function summary() {
        ...
        if (typeof fetch !== "undefined") {
            // Slice a few off the end to show how the
            // last page looks when it's not full.
            const url2 = `${link}/Get/Dosing/Summary/Summary_filter.php?date_get1=${start}&&date_get2=${stop}&&Raw_get=${S_Raw}`;
            fetch(url2)
                .then((response) => response.json())
                .then((json) => (items_2 = json.slice(0)));
        }
    ...
    function exportFile() {
        ...
        const headersItemshead = [`Dosing Material Usage Report Range= ${start} - ${stop} RawName=${S_Raw} Line Batch=${S_line} JobNo=${S_JobNo} Recipe=${S_Recipe}`];
        const headersItemsname = ["Dosing Material Usage Summary"];
        const headersItems2 = [
            "Raw Name",
            "Sum Weight SP",
            "Sum Weight Actual",
        ...
        writeFileXLSX(wb, `Dosing Material Usage Report ${S_JobNo}.xlsx`);
    }
</script>
...
<div class="paper-container">
    <div class="bar">
        ...
        <p class="namebar">
            เงื่อนไขการดึงข้อมูลรายงาน Dosing Material Usage
        </p>
    </div>
    <Paper>
        <Content>
            <div class="">
                ...
                <div class="flex flex-row w-full pl-10 pr-20">
                    ...
                    <button
                        ...
                        type="button"
                        class="flex w-1/2 justify-center text-white bg-[#60bfee] hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-small rounded-lg text-sm px-3 py-1.5 text-center items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    >
                        <svg
                            aria-hidden="true"
    ...
    <div class="table-wrapper mt-10">
        ...
        {#if processingItems === true}
            ...
            line-height: 1.75rem;">RecipeName</Cell
                        ...
                        <Cell
                            style="padding: 0;  text-align: center; vertical-align: middle; font-size: 1.125rem; 
            line-height: 1.75rem;">Dosing No.</Cell
                        >
                        <Cell
                            style="padding: 0;  text-align: center; vertical-align: middle; font-size: 1.125rem; 
                    ...
                    {#each slice as item }
                        <Row style=" line-height: 1rem; height: 20px;">
                            ...
                            <Cell
                                ...
                                >{item.WeightSP == null
                                    ? ""
                                    : Number(item.WeightSP).toFixed(2)}</Cell
                            >
                            <Cell
                                style="margin: 0; padding: 0; text-align: center; vertical-align: middle; line-height: 1rem; height: 1.5rem;"
                                >{item.WeightActual == null
                                    ? ""
                                    : Number(item.WeightActual).toFixed(2)}</Cell
                            >
                            <Cell
                                style="margin: 0; padding: 0; text-align: center; vertical-align: middle; line-height: 1rem; height: 1.5rem;"
                                >{item.SumWeightDif == null
                                    ? ""
                                    : Number(item.SumWeightDif).toFixed(2)}</Cell
                            >
                            <Cell
                                style="margin: 0; padding: 0; text-align: center; vertical-align: middle; line-height: 1rem; height: 1.5rem;"
                                >{item.Remark}</Cell
                        ...
                        <Row style=" line-height: 1rem; height: 20px;">
                            ...
                            <Cell
                                ...
                                >{item_2.SumWeightSP == null
                                    ? ""
                                    : Number(item_2.SumWeightSP).toFixed(2)}</Cell
                            >
                            <Cell
                                style="text-align: center; vertical-align: middle; line-height: 1rem; height: 1.25rem;"
